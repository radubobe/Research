...
dictOperatorTypes = {
        ...
        'FUNCTION_FLOOR': OperatorType.floor,
        'FUNCTION_SIGN': OperatorType.sign,
        }
...
class NumericalPsystem():
    ...
    def runSimulationStep(self, debug=False):
        ...
        for membraneName in self.H:
            ...
            if debug:
                print("Production for membrane %s" % membraneName)
            ...
            if (len(membrane.enzymes) == 0):
                ...
                for prgNr, prg in enumerate(membrane.programs):
                    if (prg.isActivatedByEnzyme()):
                        ...
                        if debug:
                            print("\tProgram %d activated by enzyme %s" % (prgNr, prg.enzyme.name))
...
class ProductionFunction():
    ...
    def evaluate(self):
        ...
        for item in self.items:
            ...
            elif (item == OperatorType.floor):
                # evaluate the function
                self.postfixStack.append( math.floor(self.postfixStack.pop()) )

            elif (item == OperatorType.sign):
                # evaluate the function
                self.postfixStack.append( math.copysign(1, self.postfixStack.pop()) )
...
def tokenize(code):
    ...
    token_specification = [
        ...
        ('FUNCTION_FLOOR', r'floor'),  # absolute value (modulus) function
        ('FUNCTION_SIGN', r'sign'),  # absolute value (modulus) function
